        ...
        if (expanded) {
          if (productBound) {
            ...
            var subDetailsProductful = BlocBuilder<GroceryListBloc, GroceryListState>(
              ...
              buildWhen: (previous, current) => current is PrototypeChangedState && current.previous.id == model.boundPrototype.id,
                              ...
                              child: BeautifulTextField(
                                ...
                                onEditingComplete: (state) {
                                  ...
                                  bloc.updatePrototype(
                                      model.boundPrototype.copyWith(quantization: value, quantizationDecimalNumbersAmount: dot == -1 ? 0 : state.controller.text.length - 1 - dot));
                                },
                              ),
                            ),
                            SizedBox(width: 20),
            ...
            children: [
              ...
              if (!(expanded && !productBound))
                BlocBuilder<GroceryListBloc, GroceryListState>(
                    buildWhen: (previous, current) => current is ItemsChangedState && current.contains(model.id) && current.previousOfId(model.id).title != bloc.getItemOfId(model.id, listId).title,
                    cubit: bloc,
                    builder: (context, state) {
                      model = bloc.getItemOfId(model.id, listId) ?? fallbackModel;
